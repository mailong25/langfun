...
SUPPORTED_MODELS_AND_SETTINGS = {
    ...
    'gpt-3.5-turbo-instruct': pg.Dict(rpm=10000, tpm=2000000),
    # Legacy GPT-3.5 models (deprecated)
...
class OpenAI(lf.LanguageModel):
  ...
  @classmethod
  def dir(cls):
    # Note: This would need to be updated to use the new client
    # return client.models.list() 
    raise NotImplementedError("Model listing needs to be updated for v1 API")
  ...
  def _complete_batch(
    ...
    def _openai_completion(prompts):
      ...
      for choice in response.choices:
        samples_by_index[choice.index].append(
            lf.LMSample(choice.text.strip(), score=choice.logprobs.token_logprobs[0] if choice.logprobs else 0.0)
        )
  ...
  def _chat_complete_batch(
    ...
    def _openai_chat_completion(prompt: lf.Message):
      if self.multimodal:
        content = []
        ...
        if choice.logprobs:
          logprobs = [
              (
                  t.token,
                  t.logprob,
                  [(tt.token, tt.logprob) for tt in t.top_logprobs] if t.top_logprobs else [],
              )
...
class Gpt35TurboInstruct(OpenAI):
  """GPT-3.5 Turbo Instruct - similar to legacy completion models."""
